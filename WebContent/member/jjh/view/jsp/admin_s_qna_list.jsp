	...
	<main role="main" class="container" style="margin-top : 60px">
		...
		<div class="table-responsive">
			...
			<table id="comment" class="table table-bordered table-sm">
				...
				<c:if test="${!empty s_qna_list}">
						<c:forEach var = "pqna" items = "${s_qna_list}">
						<tr>
							<td>${pqna.qid}</td>
							<td>${pqna.qtitle}</td>
							...
							<td>${pqna.qhit}</td>
							<td>${pqna.status}</td>
							<td><form method="post" action="/ShoesWebSite/UserQna_answerForm">
		...
		<ul class="pager">
			<c:if test="${pageGroup.beforePage}">
				<li><a href="/ShoesWebSite/admin_UserQnaList?reqPage=${(pageGroup.groupStartNumber)-1}">Previous</a></li>
			</c:if>
			<c:forEach var = "Page" begin="${pageGroup.groupStartNumber}" end="${pageGroup.groupEndNumber}">
				<li>
					<a href = "/ShoesWebSite/admin_UserQnaList?reqPage=${Page}">${Page}</a>
				</li>
			</c:forEach>
			<c:if test="${pageGroup.afterPage}">
				<li><a href="/ShoesWebSite/admin_UserQnaList?reqPage=${(pageGroup.groupEndNumber)+1}">Next</a></li>
			</c:if>
		</ul>
